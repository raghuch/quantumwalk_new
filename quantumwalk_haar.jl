...
function calcprobdensity!(a_up, a_down, mysettings)
    ...
    for k = 1:1:iter
        ...
        #H = createHaarUnitaryMatrices(num_steps)
...
#To find the final probability vs position plot, we just need prob at t = num_steps, i.e. prob[:,num_steps]
    ...
    if mysettings.isdemo == true
        ...
        draw(PDF("probdistdemo_haar.pdf", 6inch, 3inch), plot_probdist)
        ...
        draw(PDF("variance_haar.pdf", 6inch, 3inch), plot_variance)
...
#Plot of the probability distribution, as a function of position on the lattice
    ...
    pdfplots = Gadfly.draw(PDF("randomwalkplots_haar.pdf", 6inch, 6inch), vstack(plot_variance, plot_probdist))
=#
end


function quantumwalk_haar(isdemo::Bool)
#Start Here

    if isdemo == true  #Only a demo
        mysettings1 = Settings(1, 100, 0, true) 
        #e.g. '1' in the last but one parameter  = particle initially has 'spin-UP'
        walk(mysettings1)
    else
        mysettings2 = Settings(10, 100, 0, false)
